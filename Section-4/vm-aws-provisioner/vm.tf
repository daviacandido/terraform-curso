...
resource "aws_instance" "vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo security_group_id : ${data.terraform_remote_state.vpc.outputs.security_group_id} >> /tmp/security_group_id.txt"
    ]
  }

  provisioner "file" {
    source      = "test.txt"
    destination = "/tmp/exemplo-provisioner-file.txt"
  }
